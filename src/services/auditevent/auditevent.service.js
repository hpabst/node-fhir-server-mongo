...
module.exports.create = (args, context, logger) => new Promise((resolve, reject) => {
	...
	auditevent_resource.meta = new Meta({versionId: '1', lastUpdated: moment.utc().format('YYYY-MM-DDTHH:mm:ssZ')});
	// TODO: set meta info
    
	// TODO: save record to database
    let db = globals.get(CLIENT_DB);
	let collection = db.collection(`${COLLECTION.AUDITEVENT}_${base_version}`);
	let doc = JSON.parse(JSON.stringify(auditevent_resource.toJSON()));
	Object.assign(doc, {id: id});
	let history_doc = Object.assign({}, doc);
	Object.assign(doc, {_id: id});
	// Insert our AuditEvent record:
	collection.insertOne(doc, (err) => {
		if (err) {
			logger.error('Error with AuditEvent.create: ', err);
			return reject(err);
		}

		//Save the resource to history
		let history_collection = db.collection(`${COLLECTION.AUDITEVENT}_${base_version}_History`);

		//Insert our observation record to history but don't assign _id.
		return history_collection.insertOne(history_doc, (err2) => {
			if (err2) {
				logger.error('Error with AuditEventHisotry.create', err2);
				return reject(err2);
			}
